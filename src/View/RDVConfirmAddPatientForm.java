...
public class RDVConfirmAddPatientForm extends JFrame {
	public long numPatient,selectedRDV;
	...
	/*-- Constructor --*/
	public RDVConfirmAddPatientForm(long selectedRDV, long numPatient) {
		this.selectedRDV = selectedRDV;
		this.numPatient = numPatient;
		
		setBounds(100, 100, 400, 555);
		setTitle("Ajouter nouveau patient");
		setResizable(false);
		...
		lblHeader.setBounds(20, 10, 344, 128);
		...
		btnConfirm.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				String  nom = nomField.getText(),
						...
						sexe = sexeField.getSelectedItem().toString().trim(),
						adresse = adresseField.getText(),
						telephone = telField.getText();
				Date dateNaissance = Calender.getDate();
								
				Patient pat =  new Patient(numPatient, nom, prenom, CIN, sexe, adresse, dateNaissance, telephone);
				if (pat.Modifier()) {
					RDV rdv = new RDV(selectedRDV);
					rdv.setNumPatient(numPatient);
					if(rdv.confirmerVisite() && rdv.Supprimer()) {
						JOptionPane.showMessageDialog(null, "La visite a bien ete confirmer", "Confirmation du visite", 2);
						dispose();
					}
					else JOptionPane.showMessageDialog(null, "Echec d'insertion!", "Confirmation du visite", JOptionPane.OK_OPTION);
				}
				else JOptionPane.showMessageDialog(null, "Echec d'insertion!", "Confirmation du visite", JOptionPane.OK_OPTION);
			}
		});
	...
	/*-- Methods --*/
	public static void launch(long selectedRDV, long numPatient) {
		RDVConfirmAddPatientForm frame = new RDVConfirmAddPatientForm(selectedRDV, numPatient);
